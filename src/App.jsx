...
const CSS_VARS = `
  :root {
    --bg:#f5f3ff; --card:#ffffff; --card2:#f9f7ff;
    --border:#f3e8ff; --border2:#ddd6fe; --border3:#e9d5ff; --border4:#f3f4f6;
    --text:#1f2937; --text2:#374151; --text3:#6b7280; --text4:#9ca3af;
    --thead-bg:#f5f3ff; --thead-c:#5b21b6;
    --inp:#ffffff; --btn2-bg:#f5f3ff; --bsm-bg:#f5f3ff;
    --shadow:rgba(124,58,237,.08);
    --tab-border:#e9d5ff; --tab-bg:#ffffff;
    --accent-soft:#f5f3ff; --accent-soft-c:#7c3aed;
  }
  [data-theme="dark"] {
    --bg:#0d0d1a; --card:#16162e; --card2:#111128;
    --border:#252050; --border2:#3a306a; --border3:#2a2058; --border4:#1e1e38;
    --text:#e2e8f0; --text2:#cbd5e1; --text3:#94a3b8; --text4:#64748b;
    --thead-bg:#1a1042; --thead-c:#a78bfa;
    --inp:#0e0e22; --btn2-bg:#1a1040; --bsm-bg:#1a1040;
    --shadow:rgba(0,0,0,.3);
    --tab-border:#252050; --tab-bg:#16162e;
    --accent-soft:#1e1040; --accent-soft-c:#a78bfa;
  }
`;

function lsGet(key, def) { try { const v=localStorage.getItem(key); return v?JSON.parse(v):def; } catch { return def; } }
function lsSet(key, value) { try { localStorage.setItem(key, JSON.stringify(value)); } catch(e) { console.warn(e); } }
        ...
        <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13 }}>
          ...
          <tbody>
            {[["🧂 Insumos / 📋 Fichas",true,false,false],["📥 Compras",true,true,false],["📦 Estoque",true,true,false],["🏭 Produção",true,true,false],["💰 Preços — Editar",true,false,false],["💰 Preços — Visualizar",true,false,true],["📝 Pedidos",true,true,true],["🛒 Vendas",true,true,false],["✅ Confirmar Pedidos",true,true,false],["🏢 Despesas",true,false,false],["📊 DRE / 🎯 Dashboard",true,false,false],["👥 Gerenciar Usuários",true,false,false]].map(([label,adm,comp,vend])=>(
              <tr key={label}><td style={{ ...S.td,fontWeight:500 }}>{label}</td>{[adm,comp,vend].map((v,i)=><td key={i} style={{ ...S.td,textAlign:"center" }}>{v?"✅":<span style={{ color:"var(--border4)" }}>—</span>}</td>)}</tr>
            ))}
          </tbody>
        </table>
      </Card>
...
function InsumosDefTab({ idef, setIdef }) {
  ...
  return(<>
    <Card title="➕ Cadastrar Insumo">
      <p style={{ margin:"0 0 14px",fontSize:13,color:"var(--text3)" }}>Cadastre os ingredientes usados na produção. O custo será definido ao registrar compras.</p>
      <G cols="3fr 1fr auto">
    ...
    <Card title={`📋 Insumos (${idef.length})`}>
      <table style={{ width:"100%",borderCollapse:"collapse",fontSize:14 }}>
        ...
        <tbody>
          {!idef.length&&<tr><td colSpan={3} style={{ ...S.td,textAlign:"center",color:"var(--text4)",padding:32 }}>Nenhum insumo cadastrado. Adicione acima 👆</td></tr>}
          {idef.map(i=><tr key={i.id}><td style={{ ...S.td,fontWeight:500 }}>{i.nome}</td><td style={S.td}>{i.unidade}</td><td style={S.td}><button style={S.bsm} onClick={()=>editar(i)}>✏️</button><button style={{ ...S.bsm,color:"#ef4444",marginLeft:6 }} onClick={()=>remover(i.id)}>🗑️</button></td></tr>)}
        </tbody>
      </table>
    </Card>
  </>);
            ...
            <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13,marginBottom:10 }}>
              ...
              <tbody>
                {f.ings.map(ing=>{const ins2=idef.find(i=>i.id===ing.iid);const qc=ins2?cvt(ing.qtd,ing.un||ins2.unidade,ins2.unidade):0;const sc=ing.un&&ing.un!==ins2?.unidade;const cm2=custMedioFn(ing.iid);return(
                  <tr key={ing.id}><td style={S.td}>{ins2?.nome||"—"}</td><td style={{ ...S.td,fontWeight:600 }}>{ing.qtd} <span style={{ color:"#7c3aed" }}>{ing.un||ins2?.unidade}</span></td><td style={{ ...S.td,color:"#7c3aed" }}>{ing.un||ins2?.unidade}</td><td style={{ ...S.td,color:"var(--text4)",fontSize:12 }}>{sc?`${qc.toFixed(4)} ${ins2?.unidade}`:"—"}</td><td style={S.td}>{cm2>0?`${fR(cm2)}/${ins2?.unidade}`:<span style={{ color:"#ef4444",fontSize:12 }}>Sem compras</span>}</td><td style={{ ...S.td,fontWeight:700,color:"#7c3aed" }}>{fR(custoIng(ing))}</td><td style={S.td}><button style={{ ...S.bsm,color:"#ef4444" }} onClick={()=>setF({...f,ings:f.ings.filter(i=>i.id!==ing.id)})}>🗑️</button></td></tr>
                );})}
    ...
    <Card title={`📦 Fichas (${fichasCalc.length})`}>
      ...
      {fichasCalc.map(p=>(
        <div key={p.id} style={{ border:"1px solid var(--border3)",borderRadius:10,padding:14,marginBottom:10 }}>
          ...
          <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>{(p.ings||[]).map(ing=>{const ins2=idef.find(i=>i.id===ing.iid);return ins2&&<span key={ing.id} style={{ fontSize:12,background:"var(--accent-soft)",color:"var(--accent-soft-c)",padding:"3px 10px",borderRadius:20 }}>{ins2.nome}: {ing.qtd} {ing.un||ins2.unidade}</span>;})}</div>
        </div>
      ))}
    </Card>
  </>);
...
function VendasTab({ vendas, setVendas, fichasCalc, getPreco, estoqueProdutoFn, idef, custMedioFn }) {
  ...
  function add(){if(!f.fichaId||!f.qtd||+f.qtd<=0)return alert("Selecione o produto e a quantidade.");if(f.usaEmbalagem&&!f.embInsumoId)return alert("Selecione o insumo de embalagem.");if(f.usaEmbalagem&&(!f.embQtd||+f.embQtd<=0))return alert("Informe a quantidade de embalagens.");const es=estoqueProdutoFn(f.fichaId);if(es<+f.qtd&&!window.confirm(`⚠️ Estoque: ${es} un. Vender mesmo assim?`))return;    setVendas([...vendas,{id:uid(),fichaId:f.fichaId,qtd:+f.qtd,data:f.data,embalagemCusto:custoEmb,embQtd:f.usaEmbalagem?+f.embQtd:0,embInsumoId:f.embInsumoId,desconto:+f.desconto||0,teleValor:f.usaTele?+f.teleValor||0:0}]);setF({...blank,data:f.data});}
  ...
  return(<>
      ...
      {!fichasCalc.length?<p style={{ color:"#d97706",background:"rgba(245,158,11,.1)",padding:10,borderRadius:8,fontSize:13 }}>⚠️ Cadastre produtos e produção primeiro.</p>:(
        <>
              ...
              <div>
                ...
                {f.usaTele&&(
                  <>
                    ...
                    {+f.teleValor>0&&<div style={{ marginTop:8,fontSize:13,background:"rgba(5,150,105,.1)",color:"#059669",padding:"6px 12px",borderRadius:8 }}>🛵 Tele entrega: <strong>+{fR(+f.teleValor)}</strong> (por pedido)</div>}
                  </>
                )}
              </div>
              <div>
          ...
          {prevRec>0&&<div style={{ display:"flex",gap:10,flexWrap:"wrap" }}>
            <span style={{ fontSize:13,color:"#059669",background:"rgba(5,150,105,.1)",padding:"5px 12px",borderRadius:8 }}>💰 Receita bruta: <strong>{fR(prevRec)}</strong></span>
            {+f.teleValor>0&&f.usaTele&&<span style={{ fontSize:13,color:"#059669",background:"rgba(5,150,105,.15)",padding:"5px 12px",borderRadius:8,border:"1px solid #86efac" }}>🛵 Tele entrega: <strong>+{fR(+f.teleValor)}</strong></span>}
            {custoEmb>0&&<span style={{ fontSize:13,color:"#d97706",background:"rgba(245,158,11,.1)",padding:"5px 12px",borderRadius:8 }}>📦 Embalagem: <strong>-{fR(custoEmb)}</strong></span>}
            {+f.desconto>0&&<span style={{ fontSize:13,color:"#ef4444",background:"rgba(239,68,68,.1)",padding:"5px 12px",borderRadius:8 }}>🏷️ Desconto: <strong>-{fR(+f.desconto)}</strong></span>}
    ...
    <G cols="repeat(5,1fr)" gap={12} mb={20}>
      ...
      <KPI label="🏷️ Total Descontos" value={fR(totDesc)} color="#ef4444"/>
      ...
      <table style={{ width:"100%",borderCollapse:"collapse",fontSize:14 }}>
        ...
        <tbody>
          ...
          {vendas.map(v=>{const fc=fichasCalc.find(p=>p.id===v.fichaId),pr=getPreco(v.fichaId),ins2=idef.find(i=>i.id===v.embInsumoId);return(
            <tr key={v.id}>
              <td style={{ ...S.td,fontWeight:500 }}>{fc?.nome||"—"}</td><td style={S.td}>{v.qtd} un</td><td style={S.td}>{fR(pr)}</td>
              <td style={{ ...S.td,fontWeight:700,color:"#059669" }}>{fR(v.qtd*pr)}</td>
              <td style={S.td}>{v.embQtd>0?<span style={{ fontSize:12 }}><span style={{ color:"#d97706",fontWeight:600 }}>📦 {v.embQtd} un</span><br/><span style={{ color:"var(--text4)",fontSize:11 }}>{ins2?.nome||""} — {fR(v.embalagemCusto)}</span></span>:<span style={{ color:"var(--border3)" }}>—</span>}</td>
              <td style={S.td}>{v.desconto>0?<span style={{ color:"#ef4444",fontWeight:600 }}>-{fR(v.desconto)}</span>:<span style={{ color:"var(--border3)" }}>—</span>}</td>
              <td style={S.td}>{v.data||"—"}</td>
              <td style={S.td}><button style={{ ...S.bsm,color:"#ef4444" }} onClick={()=>remover(v.id)}>🗑️</button></td>
            </tr>
          );})}
          {vendas.length>0&&<tr style={{ background:"rgba(5,150,105,.06)" }}><td colSpan={3} style={{ ...S.td,fontWeight:700,textAlign:"right",color:"#065f46" }}>Totais:</td><td style={{ ...S.td,fontWeight:700,color:"#059669",fontSize:15 }}>{fR(totRec)}</td><td style={{ ...S.td,color:"#d97706",fontWeight:600 }}>{totEmb>0?fR(totEmb):"—"}</td><td style={{ ...S.td,color:"#ef4444",fontWeight:600 }}>{totDesc>0?`-${fR(totDesc)}`:"—"}</td><td colSpan={2} style={S.td}/></tr>}
        </tbody>
      </table>
    </Card>
  </>);
...
function DRETab({ vendas, fichasCalc, getPreco, despesas }) {
  ...
  const vendasFiltradas=filtroAtivo?vendas.filter(v=>{if(!v.data)return false;const[y,m]=v.data.split("-");return +m===mes&&+y===ano;}):vendas;
  ...
  function exportCSV(){const rows=[["Período",periodoLabel,""],["Descrição","Valor (R$)","% Receita"],["Receita de Produtos",recBruta.toFixed(2),""],["(+) Receita Tele Entrega",recTele.toFixed(2),""],["(=) Receita Total",recTotal.toFixed(2),"100.0%"],["(-) CMV",(-cmv).toFixed(2),recTotal>0?fP(-cmv/recTotal*100):"-"],["(-) Embalagens",(-totalEmbalagens).toFixed(2),recTotal>0?fP(-totalEmbalagens/recTotal*100):"-"],["(-) Descontos",(-totalDescontos).toFixed(2),recTotal>0?fP(-totalDescontos/recTotal*100):"-"],["(=) Lucro Bruto",lucBruto.toFixed(2),fP(mbPct)],...despesas.map(d=>[d.descricao,(-d.valor).toFixed(2),recTotal>0?fP(-d.valor/recTotal*100):"-"]),["(=) Lucro Operacional",lucOp.toFixed(2),fP(moPct)]];const a=document.createElement("a");a.href="data:text/csv;charset=utf-8,"+encodeURIComponent(rows.map(r=>r.join(";")).join("\n"));a.download=`DRE_NaGarrafa_${periodoLabel.replace("/","_")}.csv`;a.click();}
  function Row({label,value,indent=0,bold=false,color,bg,sep}){return(<tr style={{ background:bg,borderTop:sep?"2px solid var(--border3)":"none" }}><td style={{ ...S.td,paddingLeft:16+indent*20,fontWeight:bold?700:400,color:color||"var(--text)",fontSize:bold?15:14 }}>{label}</td><td style={{ ...S.td,textAlign:"right",fontWeight:bold?700:500,color:color||"var(--text)",fontSize:bold?15:14 }}>{fR(value)}</td><td style={{ ...S.td,textAlign:"right",fontSize:13,color:"var(--text4)" }}>{recTotal>0?fP(value/recTotal*100):"—"}</td></tr>);}
  return(<>
    <Card title="🗓️ Filtro de Período">
      <div style={{ display:"flex",alignItems:"center",gap:12,flexWrap:"wrap" }}>
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          ...
          <label htmlFor="filtroAtivo" style={{ fontSize:14,fontWeight:600,color:"#7c3aed",cursor:"pointer" }}>Filtrar por período</label>
        </div>
        ...
        <div style={{ marginLeft:8,marginTop:16 }}><span style={{ fontSize:13,background:filtroAtivo?"var(--accent-soft)":"var(--card2)",color:filtroAtivo?"#7c3aed":"var(--text3)",padding:"6px 14px",borderRadius:20,fontWeight:600 }}>{filtroAtivo?`📅 Exibindo: ${periodoLabel}`:"📅 Exibindo: Todo o período"}</span></div>
        {filtroAtivo&&<div style={{ marginTop:16 }}><span style={{ fontSize:13,color:"#059669",background:"rgba(5,150,105,.1)",padding:"6px 14px",borderRadius:20 }}>🧾 {vendasFiltradas.length} venda(s) no período</span></div>}
      </div>
    </Card>
    <G cols="repeat(4,1fr)" gap={12} mb={20}>
      <KPI label="💵 Receita Produtos" value={fR(recBruta)} color="#059669"/>
    ...
    <Card title={`📊 DRE — ${periodoLabel}`} right={<button style={S.btn2} onClick={exportCSV}>📥 Exportar CSV</button>}>
      <table style={{ width:"100%",borderCollapse:"collapse" }}>
        ...
        <tbody>
          <Row label="(+) RECEITA BRUTA" value={recBruta} bold color="#059669" bg="rgba(5,150,105,.06)"/>
          <Row label="(-) CMV — Custo das Mercadorias Vendidas" value={-cmv} indent={1} color="#ef4444"/>
          {totalEmbalagens>0&&<Row label="(-) Custos de Embalagem" value={-totalEmbalagens} indent={1} color="#ef4444"/>}
          {totalDescontos>0&&<Row label="(-) Descontos Concedidos" value={-totalDescontos} indent={1} color="#ef4444"/>}
          <Row label="(=) LUCRO BRUTO" value={lucBruto} bold bg="var(--card2)" sep color={lucBruto>=0?"#7c3aed":"#ef4444"}/>
          ...
          <Row label="(=) LUCRO OPERACIONAL" value={lucOp} bold bg={lucOp>=0?"rgba(5,150,105,.07)":"rgba(239,68,68,.07)"} sep color={lucOp>=0?"#059669":"#ef4444"}/>
          <tr style={{ background:"var(--card2)" }}><td style={{ ...S.td,fontWeight:700,color:"#7c3aed" }}>📐 Margem Operacional</td><td style={{ ...S.td,textAlign:"right",fontWeight:700,color:moPct>=0?"#7c3aed":"#ef4444",fontSize:16 }}>{fP(moPct)}</td><td style={S.td}/></tr>
        </tbody>
      </table>
    </Card>
  </>);
...
function DashboardTab({ fichasCalc, vendas, margens, getPreco, recBruta, lucOp, totDesp, despesas }) {
  ...
  return(<>
    ...
    {!porProd.length?<div style={{ textAlign:"center",padding:"48px 24px",color:"var(--text4)",background:"var(--card)",borderRadius:12,border:"1px solid var(--border)" }}><span style={{ fontSize:52 }}>📊</span><p style={{ marginTop:12 }}>Registre vendas para visualizar os gráficos.</p></div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    lsSet("nagarrafa-theme", dark?"dark":"light");
  },[dark]);
    ...
    <div style={{ fontFamily:"system-ui,-apple-system,sans-serif",minHeight:"100vh",background:"var(--bg)" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#5b21b6,#3730a3)",color:"white",padding:"14px 24px",boxShadow:"0 4px 20px rgba(0,0,0,.25)" }}>
        <div style={{ maxWidth:1300,margin:"0 auto",display:"flex",alignItems:"center",gap:12 }}>
          ...
          <div>
            <h1 style={{ margin:0,fontSize:22,fontWeight:800,letterSpacing:"-0.5px" }}>NaGarrafa</h1>
            <p style={{ margin:0,fontSize:11,opacity:.6 }}>Estoque · Custo Médio · Produção · DRE</p>
          </div>
          <div style={{ marginLeft:"auto",display:"flex",gap:12,alignItems:"center" }}>
            ...
            <button
              ...
              style={{ background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.2)",color:"white",cursor:"pointer",borderRadius:20,padding:"6px 12px",fontSize:18,display:"flex",alignItems:"center",gap:6,transition:"all .2s" }}
            >
              {dark?"☀️":"🌙"}
              <span style={{ fontSize:12,fontWeight:500 }}>{dark?"Claro":"Escuro"}</span>
            </button>
      ...
      <div style={{ background:"var(--tab-bg)",borderBottom:"1px solid var(--tab-border)",overflowX:"auto" }}>
        <div style={{ maxWidth:1300,margin:"0 auto",display:"flex" }}>
          {visibleTabs.map(t=>(
            <button key={t.idx} onClick={()=>setTab(t.idx)} style={{ padding:"12px 12px",fontSize:13,fontWeight:500,border:"none",background:"transparent",cursor:"pointer",whiteSpace:"nowrap",borderBottom:tab===t.idx?"2px solid #7c3aed":"2px solid transparent",color:tab===t.idx?"#7c3aed":"var(--text3)",display:"flex",alignItems:"center",gap:5,transition:"color .15s" }}>
